...
static int apple_spi_word_size(S5L8702SPIState *s)
{
...
static void apple_spi_update_xfer_tx(S5L8702SPIState *s)
{
...
static void apple_spi_update_xfer_rx(S5L8702SPIState *s)
{
...
static void apple_spi_update_irq(S5L8702SPIState *s)
{
    uint32_t irq = 0;
    uint32_t mask = 0;
...
static void apple_spi_update_cs(S5L8702SPIState *s)
{
...
{
    S5L8702SPIState *s = S5L8702SPI(opaque);
    if (level) {
...
static void apple_spi_run(S5L8702SPIState *s)
{
    uint32_t tx;
    uint32_t rx;
...
static uint64_t S5L8702_spi_read(void *opaque, hwaddr addr, unsigned size)
{
    S5L8702SPIState *s = S5L8702SPI(opaque);
...
static void S5L8702_spi_write(void *opaque, hwaddr addr, uint64_t data, unsigned size)
{
    S5L8702SPIState *s = S5L8702SPI(opaque);
...
static void S5L8702_spi_reset(DeviceState *d)
{
    S5L8702SPIState *s = (S5L8702SPIState *)d;
...
static void S5L8702_spi_realize(DeviceState *dev, struct Error **errp)
{
    S5L8702SPIState *s = S5L8702SPI(dev);
    ...
    switch(s->base) {
        case 0:
            dev = ssi_create_peripheral(s->spi, TYPE_IPOD_NANO3G_NOR_SPI);
            IPodNano3GNORSPIState *nor = IPOD_NANO3G_NOR_SPI(dev);
            s->nor = nor;
            break;
        case 1:
...
static const TypeInfo S5L8702_spi_info = {
    .name          = TYPE_S5L8702SPI,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(S5L8702SPIState),
    .class_init    = S5L8702_spi_class_init,
};

static void S5L8702_spi_register_types(void)
{
    type_register_static(&S5L8702_spi_info);
}

type_init(S5L8702_spi_register_types)