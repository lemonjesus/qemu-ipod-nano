...
# Usage: ./build_nor.sh <full nor dump> <decrypted efi> <dir with jpegs and bins> <output.bin>
...
echo -n -e "\x01" | dd of=$4 bs=1 seek=$((0x9E140 + 0x8)) conv=notrunc
...
dd if=/dev/zero of=$4 bs=1 count=20 seek=$((0x9E140 + 0x1d4)) conv=notrunc
...
dd if=$4 bs=1 count=$((0x200)) skip=$((0x9E140)) | sha1sum | cut -c -40 | xxd -r -p | dd of=$4 bs=1 seek=$((0x9E140 + 0x1d4)) conv=notrunc
